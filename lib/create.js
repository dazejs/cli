...
class Create {
  constructor(program) {
    ...
    /**
     *  @var {boolean} isNext next version
     */
    this.isNext = !!program.next;
  ...
  async handle() {
    ...
    if (!this.isNext) {
      if (this.version) { // 指定版本
        ...
        const data = await this.getReleaseInfo();
        const latestVersionFilePath = path.resolve(this.releaseCachePath, `${data.tag_name}.tar.gz`);
        if (fs.existsSync(latestVersionFilePath)) { // 最新版本已经缓存
          this.createProject(latestVersionFilePath);
        } else { // 未在本地缓存找到最新版本
          this.downloadRelease(data.tarball_url, data.tag_name);
        }
      }
    } else {
      const spinner = ora('downloading project from https://github.com/dazejs/daze#master...');
      spinner.start();
      download('dazejs/daze#master', this.targetPath, { clone: false }, (err) => {
        spinner.stop();
        if (err) {
          console.log(chalk.red('Failed to download repo https://github.com/dazejs/daze#master', err));
        } else {
          this.npmInstall();
        }
      });
